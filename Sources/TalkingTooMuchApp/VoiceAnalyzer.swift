...
// Audio buffer class for storing audio samples
struct AudioBuffer {
...
class VoiceAnalyzer {
    ...
    // Amplitude thresholds for speech detection
    private let silenceThreshold: Float = 0.03
    private let speakingThreshold: Float = 0.05
    
    // Time thresholds for determining speech segments
    private let minSpeechDuration: TimeInterval = 0.3
    private let pauseDuration: TimeInterval = 0.5
    ...
    // Analyze a buffer of audio samples to determine if speech is occurring
    func analyzeSamples(_ samples: [Float], at timestamp: Date) -> Bool {
        ...
        if rms > speakingThreshold {
            ...
            if !isSpeaking {
                ...
                let newSegment = SpeechSegment(startTime: timestamp)
                speechSegments.append(newSegment)
            }
            lastSpeakingTime = timestamp
            return true
        } else if rms < silenceThreshold {
            // Check if user has been silent for longer than the pause duration
            if isSpeaking && timestamp.timeIntervalSince(lastSpeakingTime) > pauseDuration {
                isSpeaking = false
                // End the current speech segment if it exists
                if let lastIndex = speechSegments.indices.last {
                    speechSegments[lastIndex].endTime = lastSpeakingTime
                }
            }
            return false
        }
        
        // In the range between silence and speaking thresholds, maintain current state
        return isSpeaking
    }
    
    // Calculate the root mean square amplitude of audio samples
    ...
    func getTotalSpeechDuration() -> TimeInterval {
        var totalDuration: TimeInterval = 0
        
        for segment in speechSegments {
            if let endTime = segment.endTime {
                let duration = endTime.timeIntervalSince(segment.startTime)
                // Only count segments that exceed the minimum speech duration
    ...
    // Reset all speech analysis data
    func reset() {
        isSpeaking = false
...
// Structure to represent a segment of speech
